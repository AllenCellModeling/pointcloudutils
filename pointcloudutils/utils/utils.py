...
def generate_from_latent(model, z, res=16, c=None, stats_dict={}, points_orig=None,**kwargs):
    model = model.eval()
    
    threshold = np.log(0.5) - np.log(1. - 0.5)
    upsampling_steps=3 
    t0 = time.time()
    ...
    if upsampling_steps == 0:
        ...
        while points.shape[0] != 0:
            ...
            values = eval_points(
                model, pointsf, z, c, **kwargs).detach().cpu().numpy()
            values = values[0]
