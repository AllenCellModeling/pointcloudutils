...
class BaseVQVAE(ABC):
    ...
    @torch.no_grad()
    def preprocess_batch(self, images: torch.Tensor, mean: tuple = (0.485),
                         std: tuple = (0.229), training: bool = False):
        """
        ...
        if len(torch.unique(images)) != 2:
            images = normalize(images, torch.tensor(mean), torch.tensor(std))
        return images

    @torch.no_grad()
    def preprocess_visualization(self, images: torch.Tensor, mean: tuple = (0.485),
                                 std: tuple = (0.229)):
        """
