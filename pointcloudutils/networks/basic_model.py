...
class BasicModel(BaseModel):
    ...
    def __init__(
        ...
        embedding_head_key: Optional[str] = None,
    ...
    def model_step(self, stage, batch, batch_idx):
        if self.embedding_head_loss:
            rec, gt, loss_2d, embed = self.forward(batch[self.x_label])
            embed_loss = self.embedding_head_weight * self.embedding_head_loss[self.embedding_head_key](embed, batch[self.embedding_head_key])
            loss_2d = loss_2d + embed_loss
        else:
            rec, gt, loss_2d = self.forward(batch[self.x_label])
