...
from .modules import *
from pointcloudutils.networks.modules import Group_v2
import pointcloudutils.clip_attn as clip
...
class I2P_MAE(nn.Module):
    ...
    def __init__(self, group_sizes, num_groups, mask_ratio,
    encoder_depths, encoder_dims, local_radius, decoder_depths, decoder_dims, decoder_up_blocks, 
    drop_path_rate, num_heads, clip_config_feat_dim, clip_config_visual_encoder, 
    embedding_head: Optional[dict] = None, embedding_head_key: Optional[str] = None):
        super().__init__()
        ...
        self.feat_dim_2d = clip_config_feat_dim
        ...
        self.clip_model, _ = clip.load(clip_config_visual_encoder)
        self.clip_model.eval()

        self.embedding_head = nn.ModuleDict(embedding_head)
        self.embedding_head_key = embedding_head_key
    ...
    def forward(self, pts, eval=False, **kwargs):
        ...
        if self.embedding_head:
            ...
            x_vis = x_vis_list[-1]
            embed = x_vis.max(1)[0]
            embed = self.embedding_head[self.embedding_head_key](embed)
        ...
        if self.embedding_head:
            return (rec_3d, gt_3d, loss_2d, embed)
        else:
            return (rec_3d, gt_3d, loss_2d)